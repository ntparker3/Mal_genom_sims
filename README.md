...
- `nextflow.config`: This file primarily defines various profiles. Current 
  options are local, rockfish, docker, and singularity. local and rockfish 
  define process resource configurations for running in different environments. 
  docker and singularity configure Nextflow to run in the corresponding 
  container.
...
- `work`: This will be created once you run it once. This is where Nextflow puts 
  intermediates.
- `slurm`: Created on Rockfish to store Slurm logs. Look at the one matching 
  your Slurm job ID (for the main workflow) to see how things went.
